...
const getFontSpec = (jsonFile: string) => {
    try {
        ...
        let fontSpec: Record<string, any> = JSON.parse(data.toString())
        return fontSpec
    } catch (error) {
...
const genKerning = (fontSpec: Record<string, any>, font: Font) => {
    ...
    for(let i = 0; i < font.glyphs.length; i++) {
        ...
        if (exists(leftUnicode as number, fontSpec.glyphs)) {
            for (let j = 0; j < font.glyphs.length; j++) {
                let rightUnicode = font.glyphs.get(j).unicode
                if (exists(rightUnicode as number, fontSpec.glyphs)) {
                    let kern = font.getKerningValue(i, j)
                    if (kern !== 0) {
                        gatherCount++            
                        let rightGlyph = font.glyphs.get(j)
                        let specGlyph:  Record<string, any> = getSpecGlyph(rightGlyph.unicode as number, fontSpec.glyphs)
                        if (specGlyph.length !== 0) {
                            // Apply the advance ratio to 'fix' the kern values
                            // (The ratio between what msdf-atlas-gen's json and the opentype.js value)
                            let adjust = specGlyph.advance/(rightGlyph.advanceWidth as number)
                            let right = String.fromCharCode(rightGlyph.unicode as number)
        ...
        if (gatherCount !== 0) {
            ...
            let left = String.fromCharCode(leftGlyph.unicode as number)
            kerning[left] = gather
        }
        count += gatherCount
    }
